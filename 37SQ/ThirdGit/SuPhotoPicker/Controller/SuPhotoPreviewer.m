...
- (void)refreshTitle {
    [self setNavigationTitle:[NSString stringWithFormat:@"%ld / %ld", self.currentPage + 1, (unsigned long)self.dataSource.count]];
}

- (void)refreshCompleteBtnStatus {
    if ([SuPhotoCenter shareCenter].selectedPhotos.count) {
        self.sendCount.text = [NSString stringWithFormat:@"完成(%ld)",(unsigned long)[SuPhotoCenter shareCenter].selectedPhotos.count];
    }else {
